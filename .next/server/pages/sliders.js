...
const SliderForm = ({ closeModal , dataRes  })=>{
    ...
    const onSubmit = async ()=>{
        ...
        const res = await fetch(`${"http://localhost:3000"}/api/slider`, postData);
...
const MUITable = ()=>{
    ...
    const handleClick = async (id)=>{
        ...
        const res = await fetch(`${"http://localhost:3000"}/api/slider`, deleteData);
        const response = await res.json();
        console.log(response);
        rastgeleSayiUret();
    };
